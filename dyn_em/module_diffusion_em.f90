    ...
    USE module_state_description, only: p_m23, p_m13, p_m22, p_m33, p_r23, p_r13, p_r12, p_m12, p_m11, p_mh3, p_mq3, &
                                        p_r12_cc,  p_r13_cc,  p_r23_cc,                                & 
                                        p_rm11, p_rm22, p_rm33, p_rm12, p_rm13, p_rm23,                & 
                                        p_rh1, p_rh2, p_rh3, p_rq1, p_rq2, p_rq3                         
   ...
   :: nba_rij
    ...
    REAL, DIMENSION(ims:ime,kms:kme,jms:jme), INTENT(  OUT)   :: defor12_cc, defor13_cc, defor23_cc
...
IF ( config_flags%defor_opt .GT. 0 ) THEN  
    ...
    DO i = i_start, i_end
       ...
       defor12_cc(i,k,j) =  -tmpzy * (hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j)
    END DO
    END DO
    END DO
    ...
    DO k=kts,ktf
    DO i=i_start,i_end
      hatavg(i,k,j) = 0.25 *( hat(i ,k,j-1) + hat(i+1,k,j-1) + hat(i ,k,j) + hat(i+1,k,j) )
    END DO
    END DO
    END DO
    ...
    DO k=kts,ktf
    DO i=i_start,i_end
      defor12_cc(i,k,j) = defor12_cc(i,k,j) + (rdy * (hatavg(i,k,j+1) - hatavg(i,k,j)))
    END DO
    END DO
    END DO
    ...
    IF ( config_flags%defor_opt .GT. 0 ) THEN
       ...
       IF ( config_flags%sfs_opt .GT. 0 ) THEN
          ...
          tmpzx	   = 0.25 * (  &
    	   	    zx(i,k  ,j) + zx(i+1,k  ,j) +  &
    	   	    zx(i,k+1,j) + zx(i+1,k+1,j) )
          nba_rij(i,k,j,P_r12_cc) = defor12_cc(i,k,j)  + (tmpzx * (hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j)) 
          END DO
          END DO
          END DO
       ...
       DO i = i_start, i_end
          tmpzx	   = 0.25 * (  &
    	   	    zx(i,k  ,j) + zx(i+1,k  ,j) +  &
    	   	    zx(i,k+1,j) + zx(i+1,k+1,j) )
          defor12_cc(i,k,j) =  defor12_cc(i,k,j) - (tmpzx * (hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j))
       END DO
       END DO
       END DO
       ...
       DO k=kts,ktf
       DO i=i_start,i_end+1
         hatavg(i,k,j) = 0.25 *( hat(i-1,k,j) + hat(i-1,k,j+1) + hat(i,k,j) + hat(i,k,j+1) )
       END DO
       END DO
       END DO
       ...
       DO k=kts,ktf
       DO i=i_start,i_end
          defor12_cc(i,k,j) = mm(i,j) * (defor12_cc(i,k,j) + (rdx * (hatavg(i+1,k,j) - hatavg(i,k,j))))
       END DO
       END DO
       END DO   
       ...
       IF ( config_flags%sfs_opt .GT. 0 ) THEN
          ...
          DO k=kts,ktf
          DO i=i_start,i_end
             nba_rij(i,k,j,P_r12_cc) =  mm(i,j) * (nba_rij(i,k,j,P_r12_cc) - (rdx * (hatavg(i+1,k,j) - hatavg(i,k,j)))) 
          END DO
          END DO
          END DO   
...
IF ( config_flags%defor_opt .GT. 0  ) THEN  
    ...
    DO i = i_start, i_end
       tmpzx	   = 0.25 * (  &
	      zx(i,k  ,j) + zx(i+1,k  ,j) +  &
	      zx(i,k+1,j) + zx(i+1,k+1,j) )
       defor13_cc(i,k,j) = mm(i,j) * (((hatavg(i+1,k,j)-hatavg(i,k,j)) * rdx) - (tmpzx * (hat(i,k+1,j)-hat(i,k,j)) * rdzw(i,k,j)))
    END DO
    END DO
    END DO
...
IF ( config_flags%defor_opt .GT. 0  ) THEN
    ...
    DO i = i_start, i_end
      ...
      hatavg(i,kte,j) =  0.5 * (  &
			cft1 * ( u(i,ktes1,j) + u(i+1,ktes1,j) )  +  &
			cft2 * ( u(i,ktes2,j) + u(i+1,ktes2,j) ) )
    END DO
    END DO
    ...
    IF ( config_flags%sfs_opt .GT. 0 ) THEN
        ...
        IF ( config_flags%defor_opt .EQ. 1 ) THEN

            DO j = j_start, j_end
            DO k = kts, ktf
            DO i = i_start, i_end

               nba_rij(i,k,j,P_r13_cc) = ((hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j)) - defor13_cc(i,k,j) 
            END DO
            END DO
            END DO    
        ...
        IF ( config_flags%defor_opt .EQ. 2 ) THEN
           ...
           nba_rij(i,k,j,P_r13_cc) = (rdz(i,k,j)*((u(i,k,j)+u(i+1,k,j))/2.)/LOG((1./rdz(i,k,j))/config_flags%z_rough)) - defor13_cc(i,k,j)
           END DO
           END DO  
    ...
    IF ( config_flags%defor_opt .EQ. 1 ) THEN
        ...
        DO i = i_start, i_end
           ...
           defor13_cc(i,k,j) = defor13_cc(i,k,j) + ((hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j))
    ...
    IF ( config_flags%defor_opt .EQ. 2 ) THEN
        ...
        DO i = i_start, i_end
           ...
           defor13_cc(i,k,j) = defor13_cc(i,k,j) + (rdz(i,k,j)*((u(i,k,j)+u(i+1,k,j))/2.)/LOG((1./rdz(i,k,j))/config_flags%z_rough))
        END DO
        END DO  
    ...
    IF ( config_flags%defor_opt .GT. 0 ) THEN  
        ...
        DO i = i_start, i_end
           tmpzy	 =  0.25 * (  &
    		         zy(i,k  ,j) + zy(i,k  ,j+1) +  &
    		         zy(i,k+1,j) + zy(i,k+1,j+1)  )
           defor23_cc(i,k,j) = mm(i,j) * (((hatavg(i,k,j+1)-hatavg(i,k,j)) * rdy) - (tmpzy * (hat(i,k+1,j)-hat(i,k,j)) * rdzw(i,k,j)))
        END DO
        END DO
        END DO
  ...
  ENDIF 
    ...
    IF ( config_flags%defor_opt .GT. 0 ) THEN 
        ...
        DO i = i_start, i_end
          ...
          hatavg(i,kte,j) =  0.5 * (  &
			    cft1 * ( v(i,ktes1,j) + v(i,ktes1,j+1) ) +  &
			    cft2 * ( v(i,ktes2,j) + v(i,ktes2,j+1) ) )
        END DO
        END DO
        ...
        IF ( config_flags%sfs_opt .GT. 0 ) THEN
            ...
            IF ( config_flags%defor_opt .EQ. 1 ) THEN
                ...
                DO i = i_start, i_end
                       ...
                       nba_rij(i,k,j,P_r23_cc) = ((hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j)) - defor23_cc(i,k,j) 
                END DO
                END DO
                END DO 
	       ...
	       DO i = i_start, i_end
	           ...
	           nba_rij(i,k,j,P_r23_cc) = (rdz(i,k,j)*((v(i,k,j)+v(i,k,j+1))/2.)/LOG((1./rdz(i,k,j))/config_flags%z_rough)) - defor23_cc(i,k,j) 
	       END DO
	       END DO    
       ...
       IF ( config_flags%defor_opt .EQ. 1 ) THEN
           ...
           DO i = i_start, i_end
              ...
              defor23_cc(i,k,j) = defor23_cc(i,k,j) + ((hatavg(i,k+1,j)-hatavg(i,k,j)) * rdzw(i,k,j))
	   ...
	   DO i = i_start, i_end
	       ...
	       defor23_cc(i,k,j) = defor23_cc(i,k,j) + (rdz(i,k,j)*((v(i,k,j)+v(i,k,j+1))/2.)/LOG((1./rdz(i,k,j))/config_flags%z_rough))
	    END DO
	    END DO     
...
SUBROUTINE smag_km( config_flags,xkmh,xkmv,xkhh,xkhv,BN2,                      &
   ...
   IF ( config_flags%defor_opt .EQ. 0 ) THEN 
   ...
   ELSE 
       ...
       do k=kts,ktf
       do i=i_start,i_end
	  def2(i,k,j)=def2(i,k,j)+ defor12_cc(i,k,j)*defor12_cc(i,k,j)     
       ...
       do k=kts,ktf
       do i=i_start,i_end
	  def2(i,k,j)=def2(i,k,j)+ defor13_cc(i,k,j)*defor13_cc(i,k,j)
       enddo
       enddo
       enddo
       ...
       do k=kts,ktf
       do i=i_start,i_end
	  def2(i,k,j)=def2(i,k,j)+ defor23_cc(i,k,j)*defor23_cc(i,k,j)
       enddo
       enddo
       enddo       
   ...
   ELSE
      ...
      DO i = i_start, i_end
	     ...
	     if (config_flags%wall_opt == 1) then 
		 rlstar2 = (1./((c_s*deltas)**2)) + (1./((KARMAN*(z(i,k,j)-ht(i,j)))**2))

        	 xkmh(i,k,j)=max((1./rlstar2)*tmp, 1.0E-6*deltas*deltas )	
	     elseif (config_flags%wall_opt == 2) then 
		 if (k==1) then
	             cd_sqrt = KARMAN/LOG((1./(2.*rdzw(i,k,j)))/config_flags%z_rough)
	             V0 = sqrt(((u(i,k,j)+u(i+1,k,j))/2.)**2. + ((v(i,k,j)+v(i,k,j+1))/2.)**2.)+epsilon
	             xkmh(i,k,j) = KARMAN * (1./(2.*rdzw(i,k,j))) * cd_sqrt * V0 
		 else
         xkmh(i,k,j)=max(c_s*c_s*deltas*deltas*tmp, 1.0E-6*deltas*deltas )
		 endif 
	     elseif (config_flags%wall_opt == 3) then 
		 if (k==1) then
	            V0 = sqrt(((u(i,k,j)+u(i+1,k,j))/2.)**2. + ((v(i,k,j)+v(i,k,j+1))/2.)**2.)+epsilon
		    
		    tmp = V0*2.*rdzw(i,k,j)/LOG((1./(2.*rdzw(i,k,j)))/config_flags%z_rough)
		    rlstar2 = (1./((c_s*deltas)**2)) + (1./((KARMAN*(z(i,k,j)-ht(i,j)))**2))
        	    xkmh(i,k,j)=max((1./rlstar2)*tmp, 1.0E-6*deltas*deltas )				     
        	 else
                    xkmh(i,k,j)=max(c_s*c_s*deltas*deltas*tmp, 1.0E-6*deltas*deltas )	     
...
END SUBROUTINE smag2d_km
   ...
   REAL , DIMENSION( ims:ime, jms:jme), INTENT(IN   ) ::     msftx, &
    ...
    if( (config_flags%tke_drag_coefficient .gt. epsilon) .or.  &    
        (config_flags%z_rough .gt. epsilon) .or.  &                 
...
SUBROUTINE horizontal_diffusion_2 ( rt_tendf, ru_tendf, rv_tendf, rw_tendf,    &
   ...
   REAL , DIMENSION( ims:ime, kms:kme, jms:jme), INTENT(IN) ::    nwtau13,&
                                                                  nwtau23
   ...
   REAL ,                                        INTENT(IN   ) ::    rdx, &
                                                                     rdy
   INTEGER, INTENT(  IN ) :: n_nba_mij, n_nba_rij, n_rsfs_rmij 
   ...
   REAL , DIMENSION(ims:ime, kms:kme, jms:jme, n_rsfs_rmij), INTENT(INOUT) &  
   :: rsfs_rmij
   ...
   INTEGER :: im, ic, is
    ...
    CALL rsfs_horizontal_diffusion_s  ( rt_tendf, mu, config_flags, thp,        &
                                        rsfs_rmij(ims,kms,jms,P_rh1),           &
                                        rsfs_rmij(ims,kms,jms,P_rh2),           &
    ...
    IF (n_moist .ge. PARAM_FIRST_SCALAR) THEN 
      ...
      moist_loop: do im = PARAM_FIRST_SCALAR, n_moist
          ...
          CALL rsfs_horizontal_diffusion_s( moist_tendf(ims,kms,jms,im),       &
                                       mu, config_flags,                  &
                                       moist(ims,kms,jms,im),             &
                                            rsfs_rmij(ims,kms,jms,P_rq1),      &
                                            rsfs_rmij(ims,kms,jms,P_rq2),      &
                                       msftx, msfty, msfux, msfuy,        &
                                       msfvx, msfvy, xkhh, rdx, rdy,      &
                                       fnm, fnp, cf1, cf2, cf3,           &
                                       zx, zy, rdz, rdzw, dnw, dn,        &
...
SUBROUTINE horizontal_diffusion_u_2( tendency, mu, config_flags,          &
   ...
   INTEGER, INTENT(  IN ) :: n_nba_mij, n_rsfs_rmij 
...
SUBROUTINE rsfs_horizontal_diffusion_s (tendency, mu, config_flags, var,       &
                                         rsfs_h1, rsfs_h2,                      &
                                   msftx, msfty, msfux, msfuy,            &
                                   msfvx, msfvy, xkhh, rdx, rdy,          &
                                   fnm, fnp, cf1, cf2, cf3,               &
   ...
   REAL , DIMENSION( ims:ime, kms:kme, jms:jme), INTENT(IN   ) :: rsfs_h1, &
                                                                  rsfs_h2
   ...
   IF ( config_flags%rsfs_opt .LT. 6 ) THEN 
     ...
     DO i = i_start, i_end + 1
        ...
        H1(i,k,j) = H1(i,k,j)+ rsfs_h1(i,k,j)
   ...
   IF ( config_flags%rsfs_opt .LT. 6 ) THEN 
      ...
      DO i = i_start, i_end
         ...
         H2(i,k,j) = H2(i,k,j) + rsfs_h2(i,k,j)
...
END SUBROUTINE rsfs_horizontal_diffusion_s
...
SUBROUTINE vertical_diffusion_2   ( ru_tendf, rv_tendf, rw_tendf, rt_tendf,   &
   ...
   j_end   = MIN(jte,jde-1)
    ...
    IF ((config_flags%tke_drag_coefficient /= 0) .AND. (config_flags%z_rough /= 0)) THEN
       CALL wrf_error_fatal3("<stdin>",4535,&
'Please choose either tke_drag_coefficient or z_rough formulation')
    END IF 
  ...
  CASE (0) 
     ...
     IF ( ( config_flags%sfs_opt .GT. 0 ) .OR. ( config_flags%m_opt .EQ. 1 ) ) THEN 
...
'isfflx value invalid for diff_opt=2' )
  END SELECT vflux
   ...
   CALL vertical_diffusion_hs( rt_tendf,  			     &
                              nba_mij(ims,kms,jms,P_mh3),            &
                               rsfs_rmij(ims,kms,jms,P_rh3),          &
                              config_flags, var_mix, mu, xkhv,       &
  ...
  CASE (1,3) 
    ...
    IF ( ( config_flags%sfs_opt .GT. 0 ) .OR. ( config_flags%m_opt .EQ. 1 ) ) THEN 
    	...
    	DO i = i_start, i_end
 		...
 		nba_mij(i,kts,j,P_mh3) = heat_flux 
   ...
   IF (n_moist .ge. PARAM_FIRST_SCALAR) THEN 
     ...
     moist_loop: do im = PARAM_FIRST_SCALAR, n_moist
       ...
       END IF
          ...
          CALL vertical_diffusion_hs( moist_tendf(ims,kms,jms,im),         &
                                      nba_mij(ims,kms,jms,P_mq3),          &
                                          rsfs_rmij(ims,kms,jms,P_rq3),        &
  ...
  CASE (1,2,3) 
    ...
    IF ( im == P_QV ) THEN
       ...
       DO i = i_start, i_end
          ...
          nba_mij(i,kts,j,P_mq3) = moist_flux 
...
SUBROUTINE vertical_diffusion_hs( tendency,                               &
                                 mh3,                                     & 
                                 rtau,                                    &
                                 config_flags, var, mu, xkhv,             &
                                 dn, dnw, rdz, rdzw, fnm, fnp,            &
                                 doing_tke,                               &
                                 ids, ide, jds, jde, kds, kde,            &
                                 ims, ime, jms, jme, kms, kme,            &
   ...
   REAL , DIMENSION( ims:ime , kms:kme, jms:jme ) ,                       &
                                            INTENT(IN   )      ::    var, &
                                                                     rdz, &
                                                                    rdzw
   REAL, DIMENSION(ims:ime, kms:kme,jms:jme), INTENT(INOUT) ::	mh3 

   REAL, DIMENSION(ims:ime, kms:kme,jms:jme), INTENT(IN)    :: rtau 
   ...
   IF ( ( config_flags%sfs_opt .GT. 0 ) .OR. ( config_flags%m_opt .EQ. 1 ) ) THEN 
     ...
     DO i = i_start, i_end
        ...
        H3(i,ktf+1,j)=0. 
         ...
         mh3(i,ktf+1,j) = H3(i,ktf+1,j) 
         mh3(i,kts,j) = H3(i,kts,j)     
...
END SUBROUTINE vertical_diffusion_hs 
    ...
    IF ( config_flags%rsfs_opt .LT. 6 ) THEN 
      ...
      DO k = kts, ktf                                               
      DO i = i_start, i_end                                          
         titau(i,k,j) = titau(i,k,j) + mu(i,j)*rtau(i,k,j)             
      END DO                                                               
      END DO                                                               
      END DO       
    ...
    IF ( config_flags%rsfs_opt .LT. 6 ) THEN 
      ...
      DO k = kts, ktf                                                      
      DO i = i_start, i_end                                                   
         titau(i,k,j) = titau(i,k,j) + muavg(i,j)*rtau(i,k,j)         
      END DO                                                         
      END DO                                                         
      END DO                                                       
   ...
   :: ctau13
    ...
    IF ( config_flags%can_opt .EQ. 1 ) THEN 
      ...
      DO k = kts+1, ktf                                               
      DO i = i_start, i_end                                           
         titau(i,k,j) = titau(i,k,j) + muavg(i,j)*ctau13(i,k,j)
      END DO                                                      
      END DO                                                         
      END DO                                                              
    ...
    IF ( config_flags%can_opt .EQ. 1 ) THEN 
      ...
      DO k = kts+1, ktf                                                    
      DO i = i_start, i_end                                 
         titau(i,k,j) = titau(i,k,j) + muavg(i,j)*ctau23(i,k,j)                
      END DO                                                               
      END DO                                                               
      END DO                                                               
  ...
  END SELECT vflux
    ...
    ELSE 
       ...
       DO i = i_start, i_end
          ...
          xxx = xkmv(i,k,j)
          yyy = defor13_cc(i,k,j)*defor13_cc(i,k,j) + defor23_cc(i,k,j)*defor23_cc(i,k,j)
          IF ( ( yyy .GT. 1.0e-6 ) .AND. ( xxx .EQ. 0.0 ) ) xxx = MAX( xxx, 1.0e-8 )
 
          tendency(i,k,j) = tendency(i,k,j) + mu(i,j) * xxx * yyy
       ...
       DO i = i_start, i_end
         ...
         tendency(i,k,j) = tendency(i,k,j) + mu(i,j) * xkmv(i,k,j) * &
                           ( defor13_cc(i,k,j) * defor13_cc(i,k,j) + defor23_cc(i,k,j) * defor23_cc(i,k,j) )
...
SUBROUTINE calc_surf_ust_hs ( ust, hfx,                      &
                              u_2, v_2, th, rdz,             &
                              rho, tsk, cd, ch,              &
                              moist, n_moist,                &
                              dt, xtime,                     &
   ...
   REAL ,           INTENT(IN   ) ::        meso_hfx
   REAL ,           INTENT(IN   ) ::        meso_tsk
   ...
   REAL , DIMENSION( ims:ime, jms:jme), INTENT(INOUT)            :: cd
   ...
   real, parameter :: epsi    = 1.e-5
   real, parameter :: beta    = 5.
   real, parameter :: gamma   = 16.
   ...
   DO i = i_start, i_end
      ...
      u1 = 0.5*( u_2(i,kts,j) + u_2(i+1,kts,j) ) 
   
      v1 = 0.5*( v_2(i,kts,j) + v_2(i,kts,j+1) ) 
      ...
      IF (cd(i,j) .EQ. 0.0) THEN 

          cd(i,j)=vk**2/((alog(z1oz0))**2)
       
          ch(i,j)=vk**2/((alog(z1oz0t))**2)

      ENDIF



       wspd1 = SQRT(u1*u1+v1*v1 + 0.000001)
       ...
       wth0 = ch1*wspd1*(th0-th1)
   
       IF (heat_flux .NE. -999.0) wth0 = heat_flux/(cpp*rho1) 
       ...
       do while ( diff .GT. epsi ) 
          ...
          if ( ( abs(wth0) .gt. 0.0 ) .and. ( ustar .gt. 0.0 ) ) then



             molength = -((ustar**3)*t_0)/(vk*ggg*wth0)
          ...
          if ( ( abs(wth0) .eq. 0.0 ) .and. ( ustar .gt. 0.0 ) ) then
          ...
          if ( ( abs(wth0) .ge. 0.0 ) .and. ( ustar .eq. 0.0 ) ) then
          ...
          if ( ( abs(wth0) .eq. 0.0 ) .and. ( ustar .eq. 0.0 ) ) then
          ...
          if ((zeta .lt. -epsi) .and. (zeta .ge. -2.0)) then


         
            xi = 1.0/((1.0-gamma*zeta)**0.25)
            psim = alog(0.5*(1.0+xi**2)*(0.5*(1.0+xi))**2) - 2.0*atan(xi)+0.5*p_i
            psih = 2.0*alog(0.5*(1.0+xi**2))
          ...
          if ((zeta .gt. +epsi) .and. (zeta .lt. 1.0)) then
          ...
          if ((zeta .le. +epsi) .and. (zeta .ge. -epsi)) then
          ...
          if ((zeta .gt. -2.0) .and. (zeta .lt. 1.0)) then

            cdn = vk**2/((alog(z1oz0)-psim)**2)
            chn = vk**2/((alog(z1oz0t)-psim)*(alog(z1oz0t)-psih))
       ...
       IF (iter .EQ. 1000 ) THEN 
          ...
          print*,'over 1000 iterations in calc_surf_ust_hs,i,j',i,j
          print*,'cd1,cdn,ch1,chn',cd1,cdn,ch1,chn
...
200 format (a5,1x,3F20.16)

END SUBROUTINE calc_surf_ust_hs
    ...
    SUBROUTINE ustm_to_ust( ust, ustm,                            &
                            ids, ide, jds, jde, kds, kde,         &
                            ims, ime, jms, jme, kms, kme,         &
                            its, ite, jts, jte, kts, kte          )

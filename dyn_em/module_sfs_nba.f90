...
SUBROUTINE calc_smnsmn( smnsmn,                       &                
  ...
  IF ( config_flags%defor_opt .EQ. 0 ) THEN 
    ...
    DO i=i_start,i_end+1
       ...
       ss12(i,k,j) = 0.5*s12(i,k,j)
       ss13(i,k,j) = 0.5*s13(i,k,j)
       ss23(i,k,j) = 0.5*s23(i,k,j)
    ...
    DO i=i_start,i_end
       ...
       ss11c(i,k,j) = ss11(i,k,j)
       ss22c(i,k,j) = ss22(i,k,j)
       ss33c(i,k,j) = ss33(i,k,j)
  ...
  ELSE 
    ...
    DO i=i_start,i_end
       ...
       ss22c(i,k,j) = ss22(i,k,j)
       ss33c(i,k,j) = ss33(i,k,j)
       ss12c(i,k,j) = ss12(i,k,j)
       ss13c(i,k,j) = ss13(i,k,j)
       ss23c(i,k,j) = ss23(i,k,j)
  ...
  DO i=i_start,i_end
     ...
     smnsmn(i,k,j) = ss11c(i,k,j)*ss11c(i,k,j) + &
                     ss22c(i,k,j)*ss22c(i,k,j) + &
                     ss33c(i,k,j)*ss33c(i,k,j) + &
               2.0*( ss12c(i,k,j)*ss12c(i,k,j) + &
...
SUBROUTINE calc_mii( m11, m22, m33,                &
  ...
  IF ( config_flags%defor_opt .EQ. 0 ) THEN 
    ...
    DO i=i_start,i_end+1
      ...
      ss13(i,k,j) = 0.5*s13(i,k,j)
      ss23(i,k,j) = 0.5*s23(i,k,j)
      rr12(i,k,j) = 0.5*r12(i,k,j)
      rr13(i,k,j) = 0.5*r13(i,k,j)
      rr23(i,k,j) = 0.5*r23(i,k,j)
    ...
    DO i = i_start, i_end
      ...
      smnsmnc(i,k,j) = smnsmn(i,k,j)

      tkec(i,k,j) = tke(i,k,j)
  ...
  ELSE 
    ...
    DO i=i_start,i_end
      ...
      ss13(i,k,j) = 0.5*s13_cc(i,k,j)
      ss23(i,k,j) = 0.5*s23_cc(i,k,j)
      rr12(i,k,j) = 0.5*r12_cc(i,k,j)
      rr13(i,k,j) = 0.5*r13_cc(i,k,j)
      rr23(i,k,j) = 0.5*r23_cc(i,k,j)
    ...
    DO i = i_start, i_end
     ...
     ss13c(i,k,j) = ss13(i,k,j)
     ss23c(i,k,j) = ss23(i,k,j)
     rr12c(i,k,j) = rr12(i,k,j)
     rr13c(i,k,j) = rr13(i,k,j)
     rr23c(i,k,j) = rr23(i,k,j)
  ...
  IF ( config_flags%sfs_opt .EQ. 1 ) THEN 
    ...
    DO i=i_start,i_end
      ...
      rdzc = rdzw(i,k,j)
      dzc = 1.0/rdzc
      delta = ( dx*dy*dzc )**0.33333333
      a = -1.0*( cs*delta )**2

      m11(i,k,j) = a*(   2.0*sqrt( 2.0*smnsmnc(i,k,j) )*ss11c(i,k,j) &
                       + c1*(   ss11c(i,k,j)*ss11c(i,k,j)            &
      ...
      m22(i,k,j) = a*(   2.0*sqrt( 2.0*smnsmnc(i,k,j) )*ss22c(i,k,j) &
                       + c1*(   ss22c(i,k,j)*ss22c(i,k,j)            &
                              + ss12c(i,k,j)*ss12c(i,k,j)            &
                              + ss23c(i,k,j)*ss23c(i,k,j)            &
                              - smnsmnc(i,k,j)/3.0                   &
                            )                                        &
      ...
      m33(i,k,j) = a*(   2.0*sqrt( 2.0*smnsmnc(i,k,j) )*ss33c(i,k,j) &
                       + c1*(   ss33c(i,k,j)*ss33c(i,k,j)            &
                              + ss13c(i,k,j)*ss13c(i,k,j)            &
                              + ss23c(i,k,j)*ss23c(i,k,j)            &
                              - smnsmnc(i,k,j)/3.0                   &
                            )                                        &
  ...
  ELSE 
    ...
    DO i=i_start,i_end
      ...
      m11(i,k,j) = a*(   2.0*sqrt( tkec(i,k,j) )*ss11c(i,k,j)          &
                       + b*(                                           &
                               c1*(   ss11c(i,k,j)*ss11c(i,k,j)        &
                                    + ss12c(i,k,j)*ss12c(i,k,j)        &
                                    + ss13c(i,k,j)*ss13c(i,k,j)        &
      ...
      m22(i,k,j) = a*(   2.0*sqrt( tkec(i,k,j) )*ss22c(i,k,j)          &
                       + b*(                                           &
                               c1*(   ss22c(i,k,j)*ss22c(i,k,j)        &
                                    + ss12c(i,k,j)*ss12c(i,k,j)        &
                                    + ss23c(i,k,j)*ss23c(i,k,j)        &
      ...
      m33(i,k,j) = a*(   2.0*sqrt( tkec(i,k,j) )*ss33c(i,k,j)          &
                       + b*(                                           &
                               c1*(   ss33c(i,k,j)*ss33c(i,k,j)        &
                                    + ss13c(i,k,j)*ss13c(i,k,j)        &
                                    + ss23c(i,k,j)*ss23c(i,k,j)        &
...
SUBROUTINE calc_m12( m12,                          &
  ...
  IF ( config_flags%defor_opt .EQ. 0 ) THEN 
    ...
    DO i = i_start, i_end
      ...
      ss12d(i,k,j) = ss12(i,k,j)

      rr12d(i,k,j) = rr12(i,k,j)
  ...
  ELSE 
    ...
    DO i = i_start, i_end
      ...
      ss12d(i,k,j) = 0.25*( ss12(i-1,k  ,j  ) + ss12(i  ,k  ,j  ) + &
                            ss12(i-1,k  ,j-1) + ss12(i  ,k  ,j-1) )

      rr12d(i,k,j) = 0.25*( rr12(i-1,k  ,j  ) + rr12(i  ,k  ,j  ) + &
                            rr12(i-1,k  ,j-1) + rr12(i  ,k  ,j-1) )

      ss13d(i,k,j) = 0.25*( ss13(i-1,k  ,j  ) + ss13(i  ,k  ,j  ) + &
                            ss13(i-1,k  ,j-1) + ss13(i  ,k  ,j-1) )

      rr13d(i,k,j) = 0.25*( rr13(i-1,k  ,j  ) + rr13(i  ,k  ,j  ) + &
                            rr13(i-1,k  ,j-1) + rr13(i  ,k  ,j-1) )

      ss23d(i,k,j) = 0.25*( ss23(i-1,k  ,j  ) + ss23(i  ,k  ,j  ) + &
                            ss23(i-1,k  ,j-1) + ss23(i  ,k  ,j-1) )

      rr23d(i,k,j) = 0.25*( rr23(i-1,k  ,j  ) + rr23(i  ,k  ,j  ) + &
                            rr23(i-1,k  ,j-1) + rr23(i  ,k  ,j-1) )
  ...
  IF ( config_flags%sfs_opt .EQ. 1 ) THEN 
    ...
    DO i=i_start,i_end
      ...
      rdzd = 0.25*( rdzw(i,k,j) + rdzw(i-1,k,j) + rdzw(i-1,k,j-1) + rdzw(i,k,j-1) )
      dzd = 1.0/rdzd
      delta = ( dx*dy*dzd )**0.33333333
      a = -1.0*( cs*delta )**2

      m12(i,k,j) = a*(   2.0*sqrt( 2.0*smnsmnd(i,k,j) )*ss12d(i,k,j) &
                       + c1*(   ss11d(i,k,j)*ss12d(i,k,j)            &
                              + ss22d(i,k,j)*ss12d(i,k,j)            &
                              + ss13d(i,k,j)*ss23d(i,k,j)            &
                            )                                        &
                       + c2*(   ss11d(i,k,j)*rr12d(i,k,j)            &
                              - ss13d(i,k,j)*rr23d(i,k,j)            & 
                              - ss22d(i,k,j)*rr12d(i,k,j)            &
  ...
  ELSE 
    ...
    DO i=i_start,i_end 
      ...
      m12(i,k,j) = a*(   2.0*sqrt( tked(i,k,j) )*ss12d(i,k,j)     &
                       + b*(                                     &
                               c1*(   ss11d(i,k,j)*ss12d(i,k,j)  &
                                    + ss22d(i,k,j)*ss12d(i,k,j)  &
                                    + ss13d(i,k,j)*ss23d(i,k,j)  &
...
SUBROUTINE calc_m13( m13,                          &
  ...
  IF ( config_flags%defor_opt .EQ. 0 ) THEN 
    ...
    DO i = i_start, i_end
      ...
      ss13e(i,k,j) = ss13(i,k,j)

      rr13e(i,k,j) = rr13(i,k,j)
  ...
  ELSE 
    ...
    DO i = i_start, i_end
      ...
      ss12e(i,k,j) = 0.5*( fnm(k)*( ss12(i  ,k  ,j  ) + ss12(i-1,k  ,j  ) ) + &
                           fnp(k)*( ss12(i  ,k-1,j  ) + ss12(i-1,k-1,j  ) ) )

      rr12e(i,k,j) = 0.5*( fnm(k)*( rr12(i  ,k  ,j  ) + rr12(i-1,k  ,j  ) ) + &
                           fnp(k)*( rr12(i  ,k-1,j  ) + rr12(i-1,k-1,j  ) ) )

      ss13e(i,k,j) = 0.5*( fnm(k)*( ss13(i  ,k  ,j  ) + ss13(i-1,k  ,j  ) ) + &
                           fnp(k)*( ss13(i  ,k-1,j  ) + ss13(i-1,k-1,j  ) ) )

      rr13e(i,k,j) = 0.5*( fnm(k)*( rr13(i  ,k  ,j  ) + rr13(i-1,k  ,j  ) ) + &
                           fnp(k)*( rr13(i  ,k-1,j  ) + rr13(i-1,k-1,j  ) ) )

      ss23e(i,k,j) = 0.5*( fnm(k)*( ss23(i  ,k  ,j  ) + ss23(i-1,k  ,j  ) ) + &
                           fnp(k)*( ss23(i  ,k-1,j  ) + ss23(i-1,k-1,j  ) ) )

      rr23e(i,k,j) = 0.5*( fnm(k)*( rr23(i  ,k  ,j  ) + rr23(i-1,k  ,j  ) ) + &
                           fnp(k)*( rr23(i  ,k-1,j  ) + rr23(i-1,k-1,j  ) ) )
  ...
  IF ( config_flags%sfs_opt .EQ. 1 ) THEN 
    ...
    DO i=i_start,i_end
      ...
      rdze = 0.5*( fnm(k)*( rdzw(i  ,k  ,j  ) + rdzw(i-1,k  ,j  ) ) + &
                   fnp(k)*( rdzw(i  ,k-1,j  ) + rdzw(i-1,k-1,j  ) ) )
      dze = 1.0/rdze
      delta = ( dx*dy*dze )**0.33333333
      a = -1.0*( cs*delta )**2

      m13(i,k,j) = a*(   2.0*sqrt( 2.0*smnsmne(i,k,j) )*ss13e(i,k,j) &
                       + c1*(   ss11e(i,k,j)*ss13e(i,k,j)            &
                              + ss12e(i,k,j)*ss23e(i,k,j)            &
                              + ss13e(i,k,j)*ss33e(i,k,j)            &
                            )                                        &
                       + c2*(   ss11e(i,k,j)*rr13e(i,k,j)            &
                              + ss12e(i,k,j)*rr23e(i,k,j)            &
                              - ss23e(i,k,j)*rr12e(i,k,j)            &
                              - ss33e(i,k,j)*rr13e(i,k,j)            &
                            )                                        &
                     )
  ...
  ELSE 
    ...
    DO i=i_start,i_end
      ...
      b = c3*delta
       ...
       m13(i,k,j) = a*(   2.0*sqrt( tkee(i,k,j) )*ss13e(i,k,j)    &
                       + b*(                                     &
                               c1*(   ss11e(i,k,j)*ss13e(i,k,j)  &
                                    + ss12e(i,k,j)*ss23e(i,k,j)  &
                                    + ss13e(i,k,j)*ss33e(i,k,j)  &
...
SUBROUTINE calc_m23( m23,                          &
  ...
  IF ( config_flags%defor_opt .EQ. 0 ) THEN 
    ...
    DO i = i_start, i_end
      ...
      ss23f(i,k,j) = ss23(i,k,j) 

      rr23f(i,k,j) = rr23(i,k,j) 
  ...
  ELSE 
    ...
    DO i = i_start, i_end
      ...
      ss12f(i,k,j) = 0.5*( fnm(k)*( ss12(i  ,k  ,j  ) + ss12(i  ,k  ,j-1) ) + &
                           fnp(k)*( ss12(i  ,k-1,j  ) + ss12(i  ,k-1,j-1) ) )

      rr12f(i,k,j) = 0.5*( fnm(k)*( rr12(i  ,k  ,j  ) + rr12(i  ,k  ,j-1) ) +  &
                           fnp(k)*( rr12(i  ,k-1,j  ) + rr12(i  ,k-1,j-1) ) )

      ss13f(i,k,j) = 0.5*( fnm(k)*( ss13(i  ,k  ,j  ) + ss13(i  ,k  ,j-1) ) + &
                           fnp(k)*( ss13(i  ,k-1,j  ) + ss13(i  ,k-1,j-1) ) )

      rr13f(i,k,j) = 0.5*( fnm(k)*( rr13(i  ,k  ,j  ) + rr13(i  ,k  ,j-1) ) + &
                           fnp(k)*( rr13(i  ,k-1,j  ) + rr13(i  ,k-1,j-1) ) )

      ss23f(i,k,j) = 0.5*( fnm(k)*( ss23(i  ,k  ,j  ) + ss23(i  ,k  ,j-1) ) + &
                           fnp(k)*( ss23(i  ,k-1,j  ) + ss23(i  ,k-1,j-1) ) )

      ss23f(i,k,j) = 0.5*( fnm(k)*( rr23(i  ,k  ,j  ) + rr23(i  ,k  ,j-1) ) + &
                           fnp(k)*( rr23(i  ,k-1,j  ) + rr23(i  ,k-1,j-1) ) )
  ...
  IF ( config_flags%sfs_opt .EQ. 1 ) THEN 
    ...
    DO i=i_start,i_end
      ...
      rdzf = 0.5*( fnm(k)*( rdzw(i  ,k  ,j  ) + rdzw(i  ,k  ,j-1) ) + &
                   fnp(k)*( rdzw(i  ,k-1,j  ) + rdzw(i  ,k-1,j-1) ) )
      dzf = 1.0/rdzf
      delta = ( dx*dy*dzf )**0.33333333
      a = -1.0*( cs*delta )**2

      m23(i,k,j) = a*(   2.0*sqrt( 2.0*smnsmnf(i,k,j) )*ss23f(i,k,j) &
                       + c1*(   ss12f(i,k,j)*ss13f(i,k,j)            &
                              + ss22f(i,k,j)*ss23f(i,k,j)            &
                              + ss23f(i,k,j)*ss33f(i,k,j)            &
                             )                                       &
                       + c2*(   ss12f(i,k,j)*rr13f(i,k,j)            &
                              + ss22f(i,k,j)*rr23f(i,k,j)            &
                              + ss13f(i,k,j)*rr12f(i,k,j)            &
                              - ss33f(i,k,j)*rr23f(i,k,j)            &
                            )                                        &
                     )
  ...
  ELSE 
    ...
    DO i=i_start,i_end
      ...
      m23(i,k,j) = a*(   2.0*sqrt( tkef(i,k,j) )*ss23f(i,k,j)    &
                       + b*(                                     &
                               c1*(   ss12f(i,k,j)*ss13f(i,k,j)  &
                                    + ss22f(i,k,j)*ss23f(i,k,j)  &
                                    + ss23f(i,k,j)*ss33f(i,k,j) &

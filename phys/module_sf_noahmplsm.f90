...
1002      continue
  ...
  REAL, PARAMETER :: A = 4.0
    ...
    IF(OPT_RUN == 4) THEN
       ...
       END DO
       SMCTOT = SMCTOT/DZTOT
       FSAT   = MAX(0.01,SMCTOT/parameters%SMCMAX) ** 4.        
  ...
  REAL, DIMENSION(1:NFINE) :: ZFINE 
   ...
   WD1 = 0.
   DO K = 1,NSOIL
     WD1 = WD1 + (parameters%SMCMAX-SH2O(K)) * DZSNSO(K) 
   ENDDO
  ...
  REAL, PARAMETER                             :: CMIC = 0.20   
      ...
      SMPFZ  = -parameters%PSISAT*1000.*S_NODE**(-parameters%BEXP)   
     ...
     ENDDO
        ...
        IF(KWTD.LE.NSOIL)THEN    
           ...
           IF(SMC(KWTD).GT.SMCEQ(KWTD))THEN
               ...
               IF(SMC(KWTD).EQ.parameters%SMCMAX)THEN 
                      WTD=ZSOIL0(IWTD)

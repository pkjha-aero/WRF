# -*- coding: utf-8 -*-
"""
WRF to site time-series with mesoscale forcing

Created on Tue Feb 16 21:45:13 2016
...
intmicro = 1   # = 1 if you want to interpolate veritcally to a microscale column (uniform grid) defined with Vres and Ztop
               # otherwise it will ouput the meso vertical levels

Vres = 5    # Vertical resolution [m]
...
# ----------------------------------------------------------------------
# Simulation filename
dirdata = '/scratch/cdraxl/MesoMicroCoupling/TendencyForcing/2013110800_FNLcat'	# folder with wrfout* files
simID = 'neutral_FNL_notdivbyfc_1gp'	
dom = 3			# domain to extract time series from wrfout_d0[dom]*
pbl = 'YSU'
dxmeso = 3000.0		# horizontal resolution of the mesoscale grid
L = 0.0           # Length of microscale domain Lx = Ly for spatial avaraging
...
simdes = 'tend WRF3.7.1, GFS FNL, MYNN PBL scheme'
...
Nblock = 1	# use this to partition the output files in blocks. Nblock determines how many wrfout files will be post-processed per output file
...
while ifile < Nfiles:
    ...
    if idates.shape[0] > 0:    
        ...
        if (cnt == Nblock) or (ifile == Nfiles-1):
            ...
            Utends.name = 'Tendency momentum LHS term U-component' 
            Utends.units = 'm s-2'
            Utends[:] = Utend_out
            
            Vtends = f.createVariable('Vtend', 'float', ('time','z',))
            Vtends.name = 'Tendency momentum LHS term V-component' 
            Vtends.units = 'm s-2'
            Vtends[:] = Vtend_out
            
            Ugs = f.createVariable('Ug', 'float', ('time','z',))
            Ugs.name = 'Geostrophic wind U-component = Pressure gradient RHS term V-component' 
            Ugs.units = 'm s-2'
            Ugs[:] = Ug_out
            
            Vgs = f.createVariable('Vg', 'float', ('time','z',))
            Vgs.name = 'Geostrophic wind V-component = - Pressure gradient RHS term U-component' 
            Vgs.units = 'm s-2'
            Vgs[:] = Vg_out
            
            Uadvs = f.createVariable('Uadv', 'float', ('time','z',))
            Uadvs.name = 'Advective momentum RHS term U-component' 
            Uadvs.units = 'm s-2'
            Uadvs[:] = Uadv_out
            
            Vadvs = f.createVariable('Vadv', 'float', ('time','z',))
            Vadvs.name = 'Advective momentum RHS term V-component' 
            Vadvs.units = 'm s-2'
            Vadvs[:] = Vadv_out
            
            Ucors = f.createVariable('Ucor', 'float', ('time','z',))
            Ucors.name = 'Coriolis momentum RHS term U-component = V velocity component' 
            Ucors.units = 'm s-2'
            Ucors[:] = Ucor_out
            
            Vcors = f.createVariable('Vcor', 'float', ('time','z',))
            Vcors.name = 'Coriolis momentum RHS term V-component = -U velocity component' 
            Vcors.units = 'm s-2'
            Vcors[:] = Vcor_out
            
            Uphyss = f.createVariable('Uphys', 'float', ('time','z',))
            Uphyss.name = 'Vertical diffusion momentum RHS term U-component' 
            Uphyss.units = 'm s-2'
            Uphyss[:] = Uphys_out
            
            Vphyss = f.createVariable('Vphys', 'float', ('time','z',))
            Vphyss.name = 'Vertical diffusion momentum RHS term V-component' 
            Vphyss.units = 'm s-2'
            Vphyss[:] = Vphys_out

...
# ----------------------------------------------------------------------
dirdata = 'tendencies_d02_YSU'
L = 9000.0       # Length [m] of microscale domain Lx = Ly for spatial avaraging
filenc = 'GABLS3_YSU_tend_L'+ str(int(L))
dxmeso = 3000.0  # Horizontal resolution [m] of the mesoscale simulation 
dtmeso = 30.0     # temporal resolution [s]
Nav = int(np.floor(L/dxmeso))
savenpz = 0     # Save tendencies to npz file 
savenc = 1      # Save tendencies to netCDF file 

# Resample using running time averages of different window sizes
windowsize = np.array([3,21,121])   # number of (dtmeso) timesteps
...
for w in range(0,Nw):
    Uw.append(U.rolling(window = windowsize[w], center = True).mean().fillna(U))
    Vw.append(V.rolling(window = windowsize[w], center = True).mean().fillna(V))
    Ww.append(W.rolling(window = windowsize[w], center = True).mean().fillna(W))
    Thw.append(Th.rolling(window = windowsize[w], center = True).mean().fillna(Th))
    Ugw.append(Ug.rolling(window = windowsize[w], center = True).mean().fillna(Ug))
    Vgw.append(Vg.rolling(window = windowsize[w], center = True).mean().fillna(Vg))
    Uadvw.append(Uadv.rolling(window = windowsize[w], center = True).mean().fillna(Uadv))
    Vadvw.append(Vadv.rolling(window = windowsize[w], center = True).mean().fillna(Vadv))
    Thadvw.append(Thadv.rolling(window = windowsize[w], center = True).mean().fillna(Thadv))
    Ucorw.append(Ucor.rolling(window = windowsize[w], center = True).mean().fillna(Ucor))
    Vcorw.append(Vcor.rolling(window = windowsize[w], center = True).mean().fillna(Vcor))
    Uphysw.append(Uphys.rolling(window = windowsize[w], center = True).mean().fillna(Uphys))
    Vphysw.append(Vphys.rolling(window = windowsize[w], center = True).mean().fillna(Vphys))
    Utendw.append(Utend.rolling(window = windowsize[w], center = True).mean().fillna(Utend))
    Vtendw.append(Vtend.rolling(window = windowsize[w], center = True).mean().fillna(Vtend))
    ustw.append(ust.rolling(window = windowsize[w], center = True).mean().fillna(ust))
    T2w.append(T2.rolling(window = windowsize[w], center = True).mean().fillna(T2))
    TSKw.append(TSK.rolling(window = windowsize[w], center = True).mean().fillna(TSK))
    wtw.append(wt.rolling(window = windowsize[w], center = True).mean().fillna(wt))
    LHw.append(LH.rolling(window = windowsize[w], center = True).mean().fillna(LH))
    Psfcw.append(Psfc.rolling(window = windowsize[w], center = True).mean().fillna(Psfc))
    L0w.append(L0.rolling(window = windowsize[w], center = True).mean().fillna(L0))
...
figname = siteID+'_tend_'+"%.0f"%(Lav)+'km_'+"%.0f"%(tav[w])+'min_U-MB_tz.png'
...
taxis_label = 'UTC time in hours since ' + datefrom.strftime('%Y-%m-%d %H:%M') + ', $L_{avg}$ = ' + "%.1f"%(Lav) + ' km, $t_{avg}$ = ' + "%.0f"%(tav[w]) + ' min'  
        
zaxis = heights 
taxis = times
Zlevels = np.linspace(-16,16,17, endpoint=True)
zaxis_label = '$z$ [$m$]'
...
# Vertical profiles 
t0 = datetime.datetime(2006,7,2,0,0,0)  # stable LLjet
...
figname = siteID+'_tend_'+"%.0f"%(Lav)+'km_'+t0.strftime('%Y-%m-%d_%H:%M')+'_S-tav-MB_z.png'
...
for iax in range (0,4):
    ...
    ax[iax].set_xticks(np.linspace(0,15,Nticks))
...
plt.suptitle('$t$ = '+t0.strftime('%Y-%m-%d %H:%M')+', $L_{avg}$ = ' + "%.1f"%(Lav) + ' km', size = 12); 
ax[3].legend(prop={'size':9})
plt.setp([a.get_yticklabels() for a in ax[1:]], visible=False)
plt.tight_layout()
